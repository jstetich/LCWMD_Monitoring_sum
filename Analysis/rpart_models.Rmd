...
These trees perform OK, with approximately `r round(0.12769 * 0.65493,3) * 100`
percent classification error. The cross validation error, however, does not
flatten out, so there is little evidence of significant overfitting here, but
from an interpretation perspective, the model is hard to summarize.

```{ do_cp_2}
plotcp(do_tree)
```
The relative error drops fairly linearly, so there is no great point to break
